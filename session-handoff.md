...
**Phase 0 is complete.** Its exit criterion — *parity report green in CI against published reference values* — is met, and the report is published, gated and reproducible.

Issues #1–#14, #27, #28 and #30 are closed. Nothing is in flight. **The remaining step is the phase close-out itself**: a pull request from `dev` into `main`, merged, then a release tag — and only then Phase 1.

**The parity report is at [`docs/parity-report.md`](docs/parity-report.md)**: 87 claims, 87 passed, 73 identical within floating point, 10 within stated tolerance, 4 documented divergences, 7 gaps listed with their reasons. It is generated by `uv run python -m tests.parity_report` from `parity-results.json`, never edited by hand, and CI regenerates it and runs `git diff --exit-code` on it — so a number that moves fails the build even when every test still passes.

**One process slip is on the record**: #14 was committed directly to `dev` (`757ee0f`) rather than through a feature branch and pull request. The maintainer chose to leave the history as it is rather than force-push `dev`, and closed #14 by hand with a comment pointing at the commit. Every other feature this session went through a pull request.
...
**Close Phase 0**, which is a release action and was left for the maintainer:

```bash
# a pull request from dev into main, merged - never a local git merge
# then, on main:
git tag -a v0.1.0 -m "Phase 0: numerical foundation"
git push origin v0.1.0
```

`main` has received nothing since the repository was created; this is its first merge from `dev`, and `CLAUDE.md` says it only ever receives one at the end of a phase. **Check the pull request's base explicitly** — GitHub's default base is still `main` here, which is right for this one and wrong for every other pull request in the project.

After that, Phase 1 — the walking skeleton — and its first issues need writing: `PROPOSAL.md` §16 has the phase table, and nothing in `feature_list.json` covers Phase 1 yet, so that file is Phase 0's record rather than a live list once the tag lands.

The only open issue left is **#24** (R `mdatools`), which is `blocked` on R not being installed and blocks nothing. It would add a genuinely independent opinion — SIMPLS and mdatools' limits are a different lineage, where `chemotools` is another Python implementation on the same NumPy — and it would fill six of the seven gaps the report lists.
...
- **The report reproduces byte-identically across machines**, which was not obvious in advance — the printed worst-difference figures could have varied with BLAS. Two GitHub runners and this machine agree on md5 `31b0e1228eee24c0ec4910d0dd556ac9`. If that ever becomes flaky, round the printed differences rather than dropping the gate.
