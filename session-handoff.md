...
**Phase 0 is released.** `main` carries it and is tagged **`v0.1.0`** (commit `601da45`); `main` and `dev` are identical. The package version is `0.1.0` in `pyproject.toml` and `__init__.py`.

Issues #1–#14, #27, #28, #30, #35 and #38 are closed. Nothing is in flight. **The next thing is Phase 1 — the walking skeleton — and its issues are not written yet.**

**The parity report is at [`docs/parity-report.md`](docs/parity-report.md)**: 87 claims, 87 passed, 73 identical within floating point, 10 within stated tolerance, 4 documented divergences, 7 gaps published with their reasons.

### The CI gate was wrong twice before it was right — read this before touching it
...
### The caveat the report carries, and why it is worth re-reading

`kernel-pca`'s fourth verification step asked that the T² and SPE limits **match published values**. What they match is `chemotools` 0.4.3 — an open implementation pinned by version, which is what `PROPOSAL.md` §10 calls a tier-1 reference and what every scikit-learn entry in the fixture already is, but **not a number printed in a paper**. The report says so in its own words, in point 3 of *What this report claims, and what it does not*.

Specifically: the SPE limit matches exactly (1.6e-18, 2.2e-19, 5.6e-18); the T² limit matches nothing and is a documented convention difference proven to be exactly `(n+1)/n`; the beta form drawn for calibration samples has no external counterpart at all.

**The report's dagger convention exists for the same reason.** T², SPE, the cumulative explained variance curve and VIP are marked †, because their reference values are *our* formula on scikit-learn's decomposition — scikit-learn reports none of them. That is an independent decomposition, not an independent implementation, and the agreement column looks equally strong either way.
...
| `main` | identical to `dev`, tagged `v0.1.0`; receives a merge only at the end of a phase |
| Open issues | 1 — #24 |
| Latest release | `v0.1.0` on `main`, tagged 2026-08-22 |
| Feature statuses | 17 × `passing`, 1 × `blocked`, 0 × `not_started` |
...
| Verification | `uv run ruff check`, `ruff format --check`, `mypy`, `pytest` — all green (432 tests), enforced by CI on 3.12 and 3.13, plus a `git diff --exit-code` on the regenerated parity report |
...
**Phase 1: the walking skeleton.** `PROPOSAL.md` §16 describes it — FastAPI and React shell, the project/dataset/pipeline model on SQLite, CSV/XLSX/JCAMP ingest, spectra plotting with decimation, the pipeline executor with job progress, and PCA end to end.

None of it has issues yet. **Writing them is the first task**, and `feature_list.json` is now Phase 0's record rather than a live list — either start a Phase 1 list beside it or archive it, but do not silently mix the two.

Two things carried into Phase 1 from the kernels:

- **`preprocessing.from_spec` is the executor's seam.** It covers every step the schema can express, and needs two things from outside the schema: the axis for `RangeSelect` and the spectrum for `MSC(reference="supplied")` — the second of which is a schema gap and still an open question with the maintainer.
...
The only open issue is **#24** (R `mdatools`), `blocked` on R not being installed and blocking nothing. It would fill six of the seven gaps the report lists.
