...
def unsourced_entries() -> list[dict[str, Any]]:
    ...
    for name in TARGETS:
        for transform in ("snv", "msc"):
            entries.append(
                entry(
                    entry_id=f"{name}.preprocess.{transform}.external",
                    dataset=name,
                    dataset_content_hash=None,
                    algorithm="preprocess",
                    quantity=f"{transform}_corrected",
                    value=None,
                    status="unsourced",
                    comparable=False,
                    ...
                    split=None,
                    software="none available",
                    software_version="n/a",
                    citation="none - not sourced",
                    notes=(
                        "scikit-learn implements no scatter correction, and no other "
                        "reference implementation is installed here. SNV and MSC are "
                        "checked against their defining identities instead - an SNV "
                        "row has mean 0 and standard deviation 1 by construction, and "
                        "MSC of the reference spectrum returns the reference. Those "
                        "are exact properties rather than a second opinion, so this "
                        "stays a gap. chemotools, evaluated in #13, would fill it."
                    ),
                )
            )

    for name in TARGETS:
        for method in ("asls", "rubberband", "polynomial"):
            entries.append(
                entry(
                    entry_id=f"{name}.preprocess.baseline_{method}.external",
                    dataset=name,
                    dataset_content_hash=None,
                    algorithm="preprocess",
                    quantity=f"baseline_{method}",
                    value=None,
                    status="unsourced",
                    comparable=False,
                    preprocessing=[f"baseline_{method}"],
                    algorithm_variant="unknown",
                    split=None,
                    software="none available",
                    software_version="n/a",
                    citation="none - not sourced",
                    notes=(
                        "Neither SciPy nor scikit-learn implements baseline "
                        "correction, and no other implementation is installed here. "
                        "The three methods are checked against their defining "
                        "properties instead - a rubberband baseline lies on the lower "
                        "convex hull and never above the spectrum, a polynomial "
                        "baseline of degree d removes a degree-d polynomial exactly, "
                        "and AsLS reaches a fixed point of its own reweighting. Those "
                        "are exact properties rather than a second opinion, so this "
                        "stays a gap. `pybaselines` is the obvious reference and "
                        "adding it is a dependency decision, not a kernel one."
                    ),
                )
            )
