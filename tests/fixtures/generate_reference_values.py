...
PLSDA_VARIANT = (
    "Two-class PLS-DA as pls-da.md specifies it: sklearn.cross_decomposition."
    "PLSRegression(scale=False) on pre-centred X and a pre-centred {0, 1} dummy "
    "response, predictions un-centred, classes assigned at 0.5. The tallies are "
    "sklearn.metrics.confusion_matrix and accuracy_score over those assignments - "
    "an independent count, not our arithmetic on their numbers."
)

#: The class column the fixture derives (pls-da.md §9): the dataset's fixture
#: target above its median is "high", otherwise "low". Sorted, "high" < "low",
#: so "low" is the class coded 1. The parity test derives the labels by this
#: same rule rather than reading them from the fixture.


def plsda_entries(name: str, dataset: ReferenceDataset) -> list[dict[str, Any]]:
    """Two-class PLS-DA reference values on one dataset (pls-da.md §9)."""
    spectra = dataset.spectra
    centred = spectra - spectra.mean(axis=0)
    target = TARGETS[name]
    fat = dataset.targets[target]
    labels = np.where(fat > np.median(fat), "high", "low")
    classes = sorted(set(labels.tolist()))
    y = (labels == classes[1]).astype(np.float64)
    observed = y.astype(int)

    model = PLSRegression(n_components=N_COMPONENTS, scale=False).fit(centred, y - y.mean())
    predictions = np.asarray(model.predict(centred)).ravel() + y.mean()
    assigned = (predictions >= 0.5).astype(int)
    ...
    for fold in folds:
        ...
        cross_validated[test] = (
            np.asarray(fold_model.predict(spectra[test] - train_mean)).ravel() + train_y_mean
        )
    assigned_cv = (cross_validated >= 0.5).astype(int)
    ...
    common = {
        ...
        "algorithm": "plsda",
        "preprocessing": ["mean_centre_x", "mean_centre_y"],
        "algorithm_variant": PLSDA_VARIANT,
        "software": "scikit-learn",
        "software_version": SKLEARN_VERSION,
        "citation": (
            f"Generated by tests/fixtures/generate_reference_values.py against "
    ...
    coding = (
        f"Class column: 'high' where {target} > median({target}) = "
        f"{float(np.median(fat)):.4f}, else "
        f"'low'; classes sorted {classes}, so '{classes[1]}' is coded 1 (pls-da.md §3). "
        f"{int(observed.sum())} of {len(y)} samples are '{classes[1]}'."
    )

    def tally(matrix: NDArray[np.int64]) -> list[float]:
        # [TN, FP, FN, TP], rows observed and columns assigned (pls-da.md §6).
        return [float(value) for value in matrix.ravel()]

    return [
        entry(
            entry_id=f"{name}.plsda.dummy_predictions.sklearn",
            quantity="dummy_predictions",
            value=predictions.tolist(),
            split=None,
            notes=f"{coding} Continuous predictions of the dummy response, A={N_COMPONENTS}.",
            **common,
        ),
        entry(
            entry_id=f"{name}.plsda.confusion.sklearn",
            quantity="confusion",
            value=tally(confusion_matrix(observed, assigned, labels=[0, 1])),
            split=None,
            notes=f"{coding} Calibration confusion matrix flattened [TN, FP, FN, TP].",
            **common,
        ),
        entry(
            entry_id=f"{name}.plsda.accuracy.sklearn",
            quantity="accuracy",
            value=float(accuracy_score(observed, assigned)),
            split=None,
            notes=f"{coding} Calibration accuracy.",
            **common,
        ),
        entry(
            entry_id=f"{name}.plsda.confusion_cv.sklearn",
            quantity="confusion_cv",
            value=tally(confusion_matrix(observed, assigned_cv, labels=[0, 1])),
            split=split,
            notes=(
                f"{coding} Confusion matrix over the pooled held-out predictions on the "
                "stored folds, centring refitted per fold, flattened [TN, FP, FN, TP]."
            ),
            **common,
        ),
        entry(
            entry_id=f"{name}.plsda.accuracy_cv.sklearn",
            quantity="accuracy_cv",
            value=float(accuracy_score(observed, assigned_cv)),
            split=split,
            notes=f"{coding} Cross-validated accuracy on the stored folds.",
            **common,
        ),
    ]
...
def main() -> None:
    ...
    for name, dataset in datasets.items():
        ...
        entries.extend(plsda_entries(name, dataset))
